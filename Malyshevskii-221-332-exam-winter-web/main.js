...
//Таблица с маршрутами экскурсий
...
function addNewRoute(number, Eleminfo) 
{
    ...
    if (Eleminfo.description.length <= 100) 
    {
    ...
    if (Eleminfo.mainObject.length <= 100) 
    {
...
function loadRoutes(numberPage, routes) 
{
    ...
    let allRoute = document.querySelectorAll(".route");
    ...
    let thisSelectRoute = document.querySelector("[data-id='" + selectRoute + "']");
...
{
    let routes = await downloadRoutesFromServer();
...
function costCalculation(event) 
{
    ...
    if (modFirstAdit.checked || modSecAdit.checked) 
    {
        if (modFirstAdit.checked && modSecAdit.checked) 
        {
            ...
            oldPriceCopy = price;
            secondOptionCost = oldPriceCopy * 1.5 - price;
        ...
        {
            oldPrice = price;
            secondOptionCost = oldPrice * 1.5 - price;
